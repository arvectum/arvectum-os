"""P5.04 — internal/provisional integration composition facade boundary.
...
class IntegrationCompositionError(RuntimeError):
    """Base fail-closed error for the bounded P5.04 composition boundary."""
...
class IntegrationCompositionFacade:
    """Internal P5.04 facade over existing integration semantic owners.
    ...
    def __init__(
        ...
        *,
        ...
        if compatibility.product_id != contract.product_id or compatibility.product_version != contract.product_version:
            raise IntegrationCompositionContinuityError(
    ...
    def compatibility_evidence(self) -> DependencyCompatibilityReport:
        """Return immutable P5.03 evidence, not permission or provider lifecycle state."""
    ...
    def _require_declared_compatible_operation(
        ...
        *,
        ...
        matches = tuple(
            item
            for item in self._compatibility.evaluations
            if item.dependency_id == dependency_id
        )
        if len(matches) != 1:
            raise IntegrationCompositionContinuityError(
                "integration dependency must resolve to exactly one composed compatibility evaluation"
            )
        evaluation = matches[0]
        if evaluation.decision is not DependencyCompatibilityDecision.COMPATIBLE:
            raise IntegrationCompositionContinuityError(
                "integration dependency is no longer compatible under the composed Product Contract"
            )
        if evaluation.declared_contract_version != dependency_contract_version:
            raise IntegrationCompositionContinuityError(
                "integration dependency contract version drifted after facade composition"
            )
    ...
    def admit_capability(
        ...
        Compatibility/declaration continuity is checked here; current access and
        capability semantics remain owned by ``validate_capability_consumption``
        and the capability-specific surface invoked after admission.
        """
        ...
        if request.product_id != self._context.product_id or request.product_version != self._context.product_version:
            raise ProductContractScopeError(
    ...
    def start_governed_execution(
        ...
        *,
        ...
        The facade checks only exact Product Contract/dependency/operation/product
        ...
        if interaction.product_id != self._context.product_id or interaction.product_version != self._context.product_version:
            raise ProductContractScopeError(
...
def compose_integration_facade(
    *,
    ...
    """Build the bounded P5.04 facade from exact governed boundary evidence.
