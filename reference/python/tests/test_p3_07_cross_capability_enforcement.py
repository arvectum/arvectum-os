...
from arvectum_os_ref.cross_capability_enforcement import AccessRequest, CrossCapabilityEnforcementError, reconstruct_audit_for_access, resolve_document_for_access, resolve_search_hit_for_access, retrieve_knowledge_for_access, search_for_access
...
class P307CrossCapabilityEnforcementTests(unittest.TestCase):
    ...
    def _record(self, *, subject: str, version: str, semantic_type: str, organization=None, payload=()):
        ...
        return CanonicalRecord(
            subject_id=self._id("subject", subject, scope), version_id=self._id("version", version, scope), semantic_type=semantic_type,
            schema_version="1", organization=organization, authority_mode=AuthorityMode.NATIVE, authority_scope=f"{semantic_type}/state",
            accountable_owner_id=self._id("principal", "owner", scope), creation_actor=creator, created_at=datetime(2026, 8, 8, tzinfo=timezone.utc),
            provenance_refs=(creator.actual_principal.principal_id,), integrity_metadata=(("representation", "p3.07-test"),), payload=payload, lifecycle_status="Retained",
        )
    ...
    def _document(self, organization=None, *, purpose="review", rights=("read",), classification="internal"):
        ...
        artifact = ArtifactContent(self._id("artifact", "a1", organization.organization_id.value), organization, "content", "text/plain", "sha256:x", "source", HandlingConstraints(classification, purpose, rights, "retain"))
    ...
    def _pin(self, role: str) -> GovernedVersionPin:
        return GovernedVersionPin(self._id(f"{role}-subject", role), self._id(f"{role}-version", f"{role}-v1"), f"example.{role}", f"{role}/state", "Retained")
    ...
    def test_cap001_allows_only_matching_current_context(self):
        ...
        self.assertEqual(resolve_document_for_access(admitted=local, artifact_id=artifact.artifact_id, request=self.request).document_version_id, local.version_id)
    ...
    def test_cap002_filters_current_governance_context(self):
        ...
        denied = ValidatedKnowledge(self._record(subject="k2", version="k2-v1", semantic_type="platform.knowledge"), (self._id("evidence", "e2"),), KnowledgeConstraints("train", "internal", ("read",), "Current"), "valid", self._id("approval", "a2"))
        self.assertEqual(tuple(hit.source_version_id for hit in retrieve_knowledge_for_access(knowledge=(eligible, denied), request=self.request)), (eligible.version_id,))
    ...
    def test_cap004_redacts_disallowed_evidence_and_denies_foreign_organization(self):
        workflow, material, execution, result, event = (self._pin(role) for role in ("workflow", "material", "execution", "result", "event"))
        ...
        with self.assertRaises(CrossCapabilityEnforcementError):
            reconstruct_audit_for_access(manifest=manifest, request=foreign_request, evidence_constraints=constraints)
